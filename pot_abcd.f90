...
module pot_abcd
  ...
  public MLpoten_c2h2_morse,MLpoten_c2h2_7
  ...
  function mlpoten_hsoh_ref(ncoords,natoms,local,xyz,force) result(f)
   ...
   real(ark)    :: xi(6),y(6),beta(3),rho,re(6),rref(5,0:4), taueq
  ...
  ! endif
   ...
   y(5)= local(5)-re(5)
   !
   taueq = force(26)*pi/180.0_ark
   !
   y(6) = cos(rho) - cos(taueq)
...
function MLpoten_c2h2_morse(ncoords,natoms,local,xyz,force) result(f)
   ...
   real(ark)              ::  f
      ...
      real(ark)    :: alpha1,alpha2,sinalpha2,sinalpha1,tau1,tau2
      !
      integer(ik)  :: Nangles
      !
