...
!dec$ define tran_debug = 1 ! set tran_debug > 2 with small vibrational bases and small expansions only
 ...
 use timer,        only : IOstart,arraystart,arraystop,arrayminus,Timerstart,Timerstop,TimerReport,MemoryReport
...
contains
 ...
 subroutine read_eigenval(njval, jval)
    ...
    do jind = 1, njval
       ...
       do gamma = 1,sym%Nrepresen
          ...
          read(iounit,"(i8,a3)") Npolyad_t,buf500(1:3)
      ...
      if (trim(job%IOj0matel_action)=='SAVE') then
        ...
        do k1 = 1,3
          ...
          do k2 = 1,3
            ...
            !do ilevel = 1, Neigenlevels
            !  !
            !  if (job%verbose>=6) write(out,"('  ilevel = ',i)") ilevel
            !  !
            !  do ideg = 1,eigen(ilevel)%ndeg
            !    !
            !    iroot = eigen(ilevel)%iroot(ideg)
            ...
            !    !i2 = i1-1+iroot
            !    !
            !    call eigen_vib_matelem_vector(0,ilevel,iroot,Neigenlevels,Neigenroots,cdimenmax,icoeff,fcoeff,cdimen,gmat,mat_s(iroot,1:iroot))
            !    !
            !  enddo
            !  !
            !enddo
        ...
        do k1 = 1,FLNmodes
          ...
          do k2 = 1,3
            ...
            !    !i2 = i1-1+iroot
            !    !
            !    call eigen_vib_matelem_vector(1,ilevel,iroot,Neigenlevels,Neigenroots,cdimenmax,icoeff,fcoeff,cdimen,gmat,mat_s(iroot,1:iroot))
            !    !
            !  enddo
            !  !
            !enddo
 ...
 subroutine restore_rot_kinetic_matrix_elements(jrot,treat_vibration,kinetic_part,chkptIO)
   ...
   !
   integer(ik)        :: iclasses,alloc,ilevel,ib,max_deg_size,nclasses
   character(len=cl)  :: job_is
   ...
   case('end')
     !
     read(chkptIO) buf18(1:4)
     if (buf18(1:4)/='hvib'.and.buf18(1:3)/='End') then
