! Td(M)-symmetry-adapted potential energy function for XY4-type molecule
! for details see CH4 papers by S.N.Yurchenko et al.
! adapted from MLpoten_xy4_qz_f12 / MLPoten routine in pot_ch4.f90 from linearized TROVE

subroutine poten_xy4_alpha_ADF(molec, func, internal, f, cart)
  use adf
  implicit none
  ...
  integer(ik) :: i, j, k, irank
  type(adf_realq) :: dF(289)

  irank = 1

  call pot_xy4_diff_V(molec, func, internal, dF)
  ...
  do i=3, func%nparams(irank)
    ...
    f(irank) = f(irank) + func%params(i,irank)*dF(k)

  enddo

end subroutine poten_xy4_alpha_ADF


!################################################################################


! Expansion terms for "poten_xy4_alpha_ADF"

subroutine pot_xy4_diff_V(molec, func, internal, dF)
  use adf
  implicit none
  ...
  type(HM_func_type), intent(in)  :: func
  type(adf_realq), intent(in)     :: internal(molec%nmodes)
  type(adf_realq), intent(out)    :: dF(289)

  integer(ik) :: i, j, k, irank
  type(adf_realq) :: y1, y2, y3, y4, y5, y6, y7, y8, y9, s1, s2, s3, r1, r2, r3, r4
  type(adf_realq) :: xyz(molec%natoms,3), dx(molec%natoms,3), cosa12, cosa13, cosa14, cosa23, cosa24, cosa34, alpha12, alpha13, alpha14, alpha23, alpha24, alpha34
  real(ark) :: r1eq, amorse1
  ...
  elseif (trim(molec%coord_transform)=='XY4_RSYMBETA_TAU') then
    ...
    y1 = 1.0_ark-exp(-amorse1*(r1-r1eq))
    y2 = 1.0_ark-exp(-amorse1*(r2-r1eq))
    y3 = 1.0_ark-exp(-amorse1*(r3-r1eq))
    y4 = 1.0_ark-exp(-amorse1*(r4-r1eq))

    call internal_to_cartesian_xy4_symbeta_tau_ADF(molec, internal, xyz)
    ...
    write(out, '(/a,a,a)') 'poten_xy4_diff_V error: coordinate type ="',trim(molec%coord_transform), '" is not supported'
    stop
    ...
    !
!    dF(290:292) = 0.0_ark !parameters not needed - damping constants
    !
!    do i=10, 289
!      dF(i) = 1.0_ark*dF(i) !
!    enddo
    !
end subroutine pot_xy4_diff_V


!################################################################################

#ifdef _MOL_XY4_DIPOLE_

subroutine dipole_xy4_alpha_ADF(molec, func, internal, f, cart)
  use adf
  implicit none
  ...
  type(adf_realq) :: r1, r2, r3, r4, amat_len, tmat(4,3), amat(3,3), ainv(3,3), dip(3), mu(3), nu(3), term_dip(3)
  real(ark) :: r1eq, coefs_amat(3,3,adf_nterms), coefs_ainv(3,3,adf_nterms)

  integer(ik) :: i, j, k, irank, imu, nmodes, iterm
  type(adf_realq) :: dF(3,681)

  nmodes = molec%nmodes
  irank = 3

  if (.not.present(cart)) then
    write(out, '(/a)') 'dipole_xy4_alpha_ADF error: Cartesian-coordinate derivatives are not present'
    stop
  endif
  ...
  call dip_xy4_diff_mu(molec, func, internal, dF)

  dip(1:3) = 0.0_ark

  do imu=1, 3
    do iterm=2, func%nparams(imu)
      k = func%iparams(1,iterm,imu)
      dip(imu) = dip(imu) + func%params(i,imu)*dF(imu,k)
    enddo !iterm
  enddo !imu
...
end subroutine dipole_xy4_alpha_ADF


!################################################################################


! Expansion terms for "dipole_xy4_alpha_ADF"

subroutine dip_xy4_diff_mu(molec, func, internal, dF)
  use adf
  implicit none
  ...
  type(HM_func_type), intent(in)  :: func
  type(adf_realq), intent(in)     :: internal(molec%nmodes)
  type(adf_realq), intent(out)    :: dF(3,681)
  ...
  if (trim(molec%coord_transform)=='XY4_RSYMALPHA') then
    ...
    y1 = 1.0_ark*(r1-r1eq) *exp(-1.0_ark*(r1-r1eq)**2)
    y2 = 1.0_ark*(r2-r1eq) *exp(-1.0_ark*(r2-r1eq)**2)
    y3 = 1.0_ark*(r3-r1eq) *exp(-1.0_ark*(r3-r1eq)**2)
    y4 = 1.0_ark*(r4-r1eq) *exp(-1.0_ark*(r4-r1eq)**2)
  ...
  elseif (trim(molec%coord_transform)=='XY4_RSYMBETA_TAU') then
    ...
    write(out, '(/a,a,a)') 'dip_xy4_diff_mu error: coordinate type ="',trim(molec%coord_transform), '" is not supported'
    stop
    ...
    dF(1,14) = -sqrt(3._ark)*y6**2*y7/3._ark+y5*y6*y7
    ...
    dF(3,681) = -y4**6+y1**6-y3**6+y2**6
      !
 end subroutine dip_xy4_diff_mu
