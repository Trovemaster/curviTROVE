function cos_x(x) result(f)
  ...
  ! special case: cos(acos(x==+-1))
  if (trim(x%except)=='derivatives(acos(x=+-1))=Infinity') then
    f%v = x%v
  ...
  if (trim(x%except)/='') then
    write(out, '(/a,1x,a)') 'cos(f(x)) error:', trim(x%except)
  ...
  do iterm=1, adf_nterms
    ...
    do ielem=1, adf_intrf(iterm)%nelem
      ...
      if (mod(order-1,4)==0.or.mod(order-2,4)==0) prod = -prod
      ...
      do iprod=2, adf_intrf(iterm)%chrule(ielem)%nprod
        ...
        elseif (deg>1) then
          ...
          write(out, '(/a)') 'cos_x error: x**0 must not appear in the chain rule expression for derivative of cos(x)'
          stop
        endif
      enddo
...
#ifdef _ADF_NANINF_
  do iterm=1, adf_nterms
    call check_nan_inf(adf_terms(:,iterm), f%d(iterm), 'cos', x)
  enddo
#endif
