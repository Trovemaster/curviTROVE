...
subroutine test_smolyak

  integer(ik) :: quad_order_nd, nmodes, ngrids(1000), igrid, orders(1000,1000), ii(1000), smolyak_nprod, info, iprod, mode_weights(1000)
  integer(ik), allocatable :: smolyak_ind(:,:), smolyak_coef(:)

  quad_order_nd = 10

  nmodes = 3
  ngrids(1:nmodes) = 100
  do igrid=1, maxval(ngrids)
    orders(igrid,1:nmodes) = igrid
  enddo
  mode_weights(1:nmodes) = 1
  mode_weights(nmodes) = 7


  call smolyak_product(nmodes, ngrids(1:nmodes), orders(1:maxval(ngrids),1:nmodes), mode_weights(1:nmodes), quad_order_nd, 1, ii(1:nmodes), smolyak_nprod)
  ...
  call smolyak_product(nmodes, ngrids(1:nmodes), orders(1:maxval(ngrids),1:nmodes), mode_weights(1:nmodes), quad_order_nd, 1, ii(1:nmodes), smolyak_nprod, smolyak_ind, smolyak_coef)
...
subroutine smolyak_grid(nmodes, maxngrids, maxorder, maxnpoints, ngrids_1d, orders_1d, npoints_1d, points_1d, weights_1d, &!
                        mode_weights, order_nd, npoints_nd, points_nd, weights_nd)

  integer(ik), intent(in) :: nmodes, maxngrids, maxorder, maxnpoints
  integer(ik), intent(in) :: ngrids_1d(nmodes)
  integer(ik), intent(in) :: orders_1d(maxngrids,nmodes)
  integer(ik), intent(in) :: npoints_1d(maxorder,nmodes)
  real(ark), intent(in)   :: points_1d(maxnpoints,maxorder,nmodes)
  real(ark), intent(in)   :: weights_1d(maxnpoints,maxorder,nmodes)
  integer(ik), intent(in) :: mode_weights(nmodes)
  integer(ik), intent(in) :: order_nd
  integer(ik), intent(out) :: npoints_nd
  real(ark), allocatable, intent(out) :: points_nd(:,:), weights_nd(:)

  integer(ik), parameter :: npoints_incr = 10000
  integer(ik) :: imode, i, smolyak_nprod, iprod, ipoint, ipoint_, prod_nelem, info, iorder, order, ind_points(maxnpoints,nmodes), &!
                 maxnpoints_nd, np, ii(nmodes), npoints(nmodes)
  integer(ik), allocatable :: smolyak_ind(:,:), prod_ind(:,:), smolyak_coef(:)
  real(ark), allocatable :: tmp_points(:,:), tmp_weights(:)

  write(out, '(/a)') 'smolyak_grid/start: generate Smolyak sparse grid'

  write(out, '(/1x,a,1x,i4)') 'quadrature order:', order_nd

  ! estimate number of elements in Smolyak sum

  call smolyak_product(nmodes, ngrids_1d(1:nmodes), orders_1d(1:maxngrids,1:nmodes), mode_weights(1:nmodes), order_nd, 1, ii, smolyak_nprod)

  ! allocate arrays to keep Smolyak coefficients and indices of 1D quadratures

  allocate(smolyak_ind(nmodes,smolyak_nprod), smolyak_coef(smolyak_nprod), stat=info)
  if (info/=0) then
    write(out, '(/a/a,10(1x,i8))') 'smolyak_grid error: failed to allocate smolyak_ind(nmodes,smolyak_nprod), smolyak_coef(smolyak_nprod)', &
    'smolyak_nprod, nmodes =', smolyak_nprod, nmodes
    stop
  endif
  ...
  call smolyak_product(nmodes, ngrids_1d(1:nmodes), orders_1d(1:maxngrids,1:nmodes), mode_weights(1:nmodes), order_nd, 1, ii, smolyak_nprod, smolyak_ind, smolyak_coef)
  ...
  do iprod=1, smolyak_nprod
    ...
    do imode=1, nmodes
      ...
      enddo
      if (iorder<=0) then
        write(out, '(/a,1x,i3,1x,a,1x,i3)') 'smolyak_grid error: could not locate 1D quadrature of', order, 'order for mode #', imode
        write(out, '(a,1x,i3,1x,a,100(1x,i3))') 'list of available quadratures for mode #', imode, ', orders =', orders_1d(1:ngrids_1d(imode),imode)
        stop
      endif
      ...
      npoints(imode) = npoints_1d(iorder,imode)
      ind_points(1:npoints(imode),imode) = (/(ipoint, ipoint=1, npoints(imode))/)
    ...
    allocate(prod_ind(nmodes,prod_nelem), stat=info)
    if (info/=0) then
      write(out, '(/a/a,10(1x,i8))') 'smolyak_grid error: failed to allocate prod_ind(nmodes,prod_nelem)', 'nmodes, prod_nelem =', nmodes, prod_nelem
      stop
    endif
    ...
    do ipoint=1, prod_nelem
      ...
      if (ipoint_>maxnpoints_nd) then
        ...
        allocate(points_nd(nmodes,maxnpoints_nd), weights_nd(maxnpoints_nd), stat=info)
        if (info/=0) then
          write(out, '(/a/a,10(1x,i8))') 'smolyak_grid error: failed to allocate points_nd(nmodes,maxnpoints_nd), weights_nd(maxnpoints_nd)', &
          'nmodes, maxnpoints_nd =', nmodes, maxnpoints_nd
          stop
        endif
        points_nd = 0
      ...
      points_nd(1:nmodes,ipoint_) = (/( points_1d(prod_ind(imode,ipoint),iorder,imode), imode=1, nmodes )/)
      weights_nd(ipoint_) = product( (/( weights_1d(prod_ind(imode,ipoint),iorder,imode), imode=1, nmodes )/) ) * real(smolyak_coef(iprod),ark)
  ...
  deallocate(smolyak_ind, smolyak_coef)
  ...
  if (maxnpoints_nd/=npoints_nd) then
    ...
    allocate(points_nd(nmodes,npoints_nd), weights_nd(npoints_nd), stat=info)
    if (info/=0) then
      write(out, '(/a/a,10(1x,i8))') 'smolyak_grid error: failed to allocate points_nd(nmodes,npoints_nd), weights_nd(npoints_nd)', &
      'nmodes, npoints_nd =', nmodes, npoints_nd
      stop
    endif
    points_nd(1:nmodes,1:npoints_nd) = tmp_points(1:nmodes,1:npoints_nd)
  ...
  write(out, '(/a)') 'smolyak_grid/done'

end subroutine smolyak_grid
