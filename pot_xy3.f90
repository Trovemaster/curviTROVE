...
module pot_xy3
  ...
  public MLspinspin_xy3_mb,MLsymfunc_xy3_morbid_10
  public MLpoten_oh3p_mep,ML_MEP_NH3,MLpoten_xy3_HSL
  public MLpoten_xy3_morbid_10_LR
  public MLpoten_ch3_schwenke
...
!
  ...
  function MLpoten_ch3_schwenke(ncoords,natoms,local,xyz,force) result(f)
   ...
   integer(ik) :: iparam, icoord
   real(ark) :: r1, r2, r3, alpha1, alpha2, alpha3, s1, s2, s3a, s3b, s4a, s4b, n1(3), n2(3), n3(3), s1_eq, angs, cm, r_eq, fac

   ! define equilibrium

   angs = 0.529177249_ark
   cm = 219474.63067_ark
   ...
   !s1 = (r1 + r2 + r3)/sqrt(3.0_ark) - s1_eq
   s1 = (r1 + r2 + r3 - 3.0_ark*r_eq)/sqrt(3.0_ark)
   s3a = (2.0_ark*r1-r2-r3)/sqrt(6.0_ark)
   s3b = (r2-r3)/sqrt(2.0_ark)

   s4a = (2.0_ark*alpha1-alpha2-alpha3)/sqrt(6.0_ark)
   s4b = (alpha2-alpha3)/sqrt(2.0_ark)

   n1 = (xyz(2,1:3)-xyz(1,1:3))!/r1
   n2 = (xyz(3,1:3)-xyz(1,1:3))!/r2
   n3 = (xyz(4,1:3)-xyz(1,1:3))!/r3

   s2 = sum(MLvector_product(n2,n3) * n1)/(r1*r2*r3)

   ! define potential

   f = 0.0
   do iparam=2, molec%parmax
     fac = product( (/( qfactorial(molec%pot_ind(icoord,iparam)), icoord=1, 6 )/) )
     f = f + force(iparam)*product( (/s1, s2, s3a, s4a, s3b, s4b/)**molec%pot_ind(1:6,iparam) )/fac
   enddo

   f = f * cm

  end function MLpoten_ch3_schwenke
...
function MLpoten_xy3_morbid_10_LR(ncoords,natoms,local,xyz,force) result(f)
   !
   integer(ik),intent(in) ::  ncoords,natoms
   real(ark),intent(in)   ::  local(ncoords)
   real(ark),intent(in)   ::  xyz(natoms,3)
   ...
   real(ark)            ::  v0,rhoe,v1,v2,v3,v4,v5,v6,rho,re14,aa1,tau
   real(ark)            ::  sinrho,alpha,coro,tau_2,cosalpha,sinphi,phi1,phi2,phi3,delta
    real(ark) :: vshort, vdamp, vlong, De1, De2, Ae1, Ae2, Gamma1, Delta1, Delta2, rad
   ...
   r34 = local(3)
      ...
      coro=(sin(rhoe)-sinrho)
      !
      if (trim(molec%potentype)=='POTEN_XY3_MORBID_10_LR_MEP') re14 = ML_mep_oh3p(rho)
      !
      y1=1.0_ark-exp(-aa1*(r14-re14))
      y2=1.0_ark-exp(-aa1*(r24-re14))
      y3=1.0_ark-exp(-aa1*(r34-re14))
  ...
  vshort = poten_xy3_morbid_10(y1,y2,y3,y4,y5,coro,force(11:))

  ! long-range potential

  vlong = De1*(y1**2 + y2**2 + y3**2) + De2*(y1**4 + y2**4 + y3**4) &!
         + exp( -Gamma1*((r14-re14)**2+(r24-re14)**2+(r34-re14)**2) ) &!
         * ( Ae1*((alpha0-alpha1)**2+(alpha0-alpha2)**2+(alpha0-alpha3)**2) &!
           + Ae2*((alpha0-alpha1)**4+(alpha0-alpha2)**4+(alpha0-alpha3)**4) )

  ! damping function

  vdamp = exp( -Delta1*((r14-re14)**2+(r24-re14)**2+(r34-re14)**2) &!
               -2.0_ark*Delta1*((r14-re14)**4+(r24-re14)**4+(r34-re14)**4) ) &!
        * exp( -Delta2*((alpha0-alpha1)**2+(alpha0-alpha2)**2+(alpha0-alpha3)**2) &!
               -2.0_ark*Delta2*((alpha0-alpha1)**4+(alpha0-alpha2)**4+(alpha0-alpha3)**4) )

  f = vshort*vdamp + vlong

 end function MLpoten_xy3_morbid_10_LR



subroutine MLsymfunc_xy3_morbid_10(rank,ncoords,natoms,local_,xyz,f)
  ...
  integer(ik) :: iatom
  real(ark) :: re14, alpha0, rhoe, aa1, r14, r24, r34, alpha1, alpha2, alpha3, alpha, sinrho, rho, y1, y2, y3, y4, y5, coro, x(natoms,3)

  ! equilibrium

  re14    = extF%coef(1,1)
  alpha0  = extF%coef(2,1)*pi/180.0_ark
  rhoe    = pi-asin(2.0_ark*sin(alpha0*0.5_ark)/sqrt(3.0_ark))
  aa1     = extF%coef(3,1)

  ! cartesian coords -> internal coords

  forall(iatom=1:4) x(iatom,:) = xyz(iatom,:) - xyz(1,:)

  r14 = sqrt(sum(x(2,:)**2))
  r24 = sqrt(sum(x(3,:)**2))
  r34 = sqrt(sum(x(4,:)**2))
  alpha3 = acos(sum(x(2,:)*x(3,:))/(r14*r24))
  alpha2 = acos(sum(x(2,:)*x(4,:))/(r14*r34))
  alpha1 = acos(sum(x(3,:)*x(4,:))/(r24*r34))
  ...
  y1 = (r14-re14)*exp(-aa1*(r14-re14)**2)
  y2 = (r24-re14)*exp(-aa1*(r24-re14)**2)
  y3 = (r34-re14)*exp(-aa1*(r34-re14)**2)
  y4 = (2.0_ark*alpha1-alpha2-alpha3)/sqrt(6.0_ark)
  y5 = (alpha2-alpha3)/sqrt(2.0_ark)

  f(1) = poten_xy3_morbid_10(y1,y2,y3,y4,y5,coro,extF%coef(4:,1))

end subroutine MLsymfunc_xy3_morbid_10
