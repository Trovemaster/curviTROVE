...
module mol_xy2
  ...
  function ML_coordinate_transform_XY2(src,ndst,direct) result (dst)
    ...
    real(ark)                 :: r1,r2,R,r12,r12e,r13e,alpha1,alpha2,theta,sintheta
    !
    !
    integer(ik) :: nsrc
    !
  ...
  subroutine ML_b0_XY2(Npoints,Natoms,b0,rho_i,rho_ref,rho_borders)
     ...
     real(ark)               :: tmat(3,3),transform0(3,3),transform1(3,3),sinrho,hx,hy,unit(3),tanphi,Inert(3)
     ...
     character(cl)           :: method
      ...
      if (Npoints/=0) then
         ...
         case('DX-RHO')
            ...
            a02 = (m1/m)
            !
         case('R-RHO','R-EXPRHO','R-RHO-Z','R12-R','R12-RHO','R13-RHO','R-PHI-RHO','R-PHI-RHO-Z')
            !
            rho_ref = 0.0_ark
            rho0 = 0.0_ark
            a02 = (m1/m)
  ...
  subroutine ML_symmetry_transformation_XY2(ioper,natoms,src,dst)
    ...
    !
    integer(ik) :: nsrc,Nrot,irot,N_Cn,ioper_
    real(ark)   :: phi,q1,q2,r,theta,phi_n,a,b,e,o,repres(sym%Noper,2,2)
    !
  ...
  subroutine ML_rotsymmetry_XY2(J,K,tau,gamma,ideg)
    ...
    case('DNH','DNH(M)')
       ...
       elseif (tau<=1.and.k<=j) then
          ...
          !endif
          !
          !if     (mod(k+2,2)==0.and.k_<=N_Cn) then 
          !    gamma = 4+2*k_-1
          !elseif (mod(k+2,2)/=0.and.k_<=N_Cn) then 
          !    gamma = 4+2*k_-1
          !elseif (mod(k+2,2)==0.and.k_>N_Cn) then 
          !    gamma = 4+2*l
          !elseif (mod(k+2,2)/=0.and.k_>N_Cn) then 
