...
!
  ...
  !
!  subroutine ME_numerov(vmax_,maxorder_,rho_b_,isingular_,npoints_,numerpoints_,drho_,poten_,mu_rr_,icoord,iperiod_,verbose_,g_numerov,energy,phi)
...
!   real(ark),intent(out) :: phi(0:npoints_,0:vmax_) !(0:npoints_,0:vmax_)
   !
   ...
   !
   integer(ik) :: vl,vr,lambda,alloc,i,rec_len,k,i_,i1,i2,info
   !
   ...
   real(ark),allocatable :: enerslot(:),enerslot_(:)
    ...
    if (verbose>=1) write (out,"(/'Numerov matrix elements calculations')")
     ...
     do vl = 0,vmax
        ...
        !
!        phi(0:npoints_,vl) = phil(0:npoints_)
        !
        do vr = vl,vmax
            !
  ...
  subroutine numerov(npoints,npoints_,step_scale,poten,mu_rr,f,enerslot)
   ...
   do v=0,vmax+1
     ...
     search_loop : do while(notfound.and.iter<itermax)
       ...
       !
       if (verbose>=5) write(out,"('eguess = ',f14.7)") eguess
       !
     ...
     if( notfound ) then
       !
        ...
        write (out,"('        v = ',i8,', eguess= ',f20.10)") v,eguess
        !
        do i = 0,maxslots
          if (enerslot(i)>poten(imin)) write (out,"('        v,ener = ',i8,f20.10)") i,enerslot(i)
        enddo
