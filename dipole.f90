...
!dec$ define dipole_debug = 0 ! set dipole_debug > 2 with small expansions only
 ...
 use accuracy,     only : hik, ik, rk, ark, cl, out, vellgt, planck, avogno, boltz, pi, small_, rad
 ...
 public Transform_rotdens_to_coord_repres,do_rotational_density,do_Angular_momentum_average
...
contains
 ...
 subroutine dm_intensity_symmvec(Jval)
    ...
    real(rk),    allocatable :: vecI(:), vecF(:),vec(:),vecPack(:)
    ...
    enddo
    !
    allocate(vecI(dimenmax_),vecF(dimenmax_),vec(dimenmax),vecPack(dimenmax), stat = info)
    !
           ...
           case('ABSORPTION')
             ...
             Idegs_loop: do idegF = 1, ndegF
                ...
                Fdegs_loop: do idegI = 1,ndegI
                  ...
                  !
                  linestr_deg(idegI,idegF) = ddot(dimenF,half_linestr(1:dimenF,indF,idegI,1),1,vec(1:dimenF),1)
                  !
    ...
    !
    deallocate(vecI, vecF, vec, vecPack, icoeffI)
 ...
 subroutine restore_vib_matrix_elements
   ...
   !
   matsize = rootsize2*3
   allocate(dipole_me(ncontr_t,ncontr_t,extF%rank),stat=alloc)
 ...
 subroutine dm_intensity(Jval)
  ...
  subroutine do_reduced_density(indI,vector,imode,jmode,kmode,density)
    ...
    integer(ik)             :: icontrI, &
      ...
      subroutine do_rotational_density2(indI,vectorI,indF,vectorF,density)
        ...
        integer(ik)             :: irootF, icontrF, icontrI, &
                                   kF, kI, tauF, tauI, ktau,&
                                   irootI,dimenI, dimenF, termI,termF

          !
          !dms_tmp = dipole_me
          !
          call TimerStart('do_rotational_density2')
          !
          density    = 0
          !
          dimenI = bset_contr(indI)%Maxcontracts
          ...
          loop_F : do irootF = 1, dimenF
            ...
            !omp end parallel do
            !
            call TimerStop('do_rotational_density2')
            !
      end subroutine do_rotational_density2
...
subroutine tav_eig(Jval)

  integer(ik), intent(in) :: Jval(:)

  integer(ik) :: nJ, jind, Jeigenvec_unit(size(Jval),sym%Nrepresen), rank, nclasses, nmodes, nlevels, Jrotmax, info, ilevel, ideg, Jrot, &
                 isym, ndeg, irec, idimen, dimen, irow, ib, ktau, icontr, iterm, nelem, ielem, isroot, k, itau, irank, eigunit, nterms, igamma, nsize, &
                 dimen0, i, j, maxnsize
  real(rk) :: beta, partfunc, energy, dtemp0, ddot
  real(rk), allocatable :: me(:,:), vec(:), psi(:,:), tav(:,:,:)
  type(DkmatT), allocatable :: ijterm(:)

  write(out, '(/a)') 'tav_eig: compute thermal average of external function'
  write(out, '(3(/1x,a,1x,es16.8))') 'temperature (K):', intensity%temperature, 'partition function:', intensity%part_func, 'ZPE:', intensity%ZPE

  beta = planck * vellgt / (boltz * intensity%temperature)  ! 1/kT
  ...
  nclasses = size(eigen(1)%cgamma)-1
  nmodes = molec%nmodes
  dimen0   = bset_contr(1)%Maxcontracts
  nlevels = Neigenlevels
  Jrotmax = maxval(eigen(1:nlevels)%jval)
  ...
  allocate(me(rank,sym%Maxdegen), psi(dimen0,0:2*Jrotmax+1), vec(maxnsize), tav(rank,nJ,sym%Nrepresen), ijterm(nJ), stat=info)
  if (info/=0) then
    write(out, '(/a/a,10(1x,i6))') 'tav_eig error: failed to allocate me(rank,sym%Maxdegen), psi(dimen0,0:2*Jrotmax+1), vec(maxnsize), tav(rank,nJ,sym%Nrepresen), ijterm(nJ)', &
    'rank, sym%Maxdegen, dimen0, maxnsize, Jrotmax, sym%Nrepresen, nJ = ', rank, sym%Maxdegen, dimen0, maxnsize, Jrotmax, sym%Nrepresen, nJ
    stop
  endif
  ...
  do jind=1, nJ
    ...
    allocate (ijterm(jind)%kmat(bset_contr(jind)%Maxsymcoeffs,sym%Nrepresen), stat=info)
    if (info/=0) then
      write(out, '(/a/a,10(1x,i6))') 'tav_eig error: failed to allocate ijterm(jind)%kmat(bset_contr(jind)%Maxsymcoeffs,sym%Nrepresen)', &
  ...
  if (job%verbose>=3) then
    write(out, '(/1x,a)') 'diagonal matrix elements'
  endif

  do ilevel=1, nlevels

    jind    = eigen(ilevel)%jind
    Jrot    = eigen(ilevel)%jval    ;  if (Jrot<intensity%J(1).or.Jrot>intensity%J(2)) cycle
    energy  = eigen(ilevel)%energy
    isym    = eigen(ilevel)%igamma
    ...
    read(eigunit, rec=irec) vec(1:nsize)
    ...
    do ideg=1, ndeg
      ...
      do idimen=1, dimen
        ...
        do ielem=1, nelem
          isroot = iterm + ielem
          dtemp0 = dtemp0 + vec(isroot)*bset_contr(jind)%irr(isym)%repres(isroot,ideg,ib)
        enddo
        psi(icontr,ktau) = dtemp0
      enddo
      ...
      do k=0, Jrot
        do itau=0, 1
           ktau = 2*k + itau
           do irank=1, rank
             call dgemv('N', dimen0, dimen0, 1.0d0, dipole_me(1:dimen0,1:dimen0,irank), dimen0, psi(1:dimen0,ktau), 1, 0.0d0, vec(1:dimen0), 1)
             me(irank,ideg) = me(irank,ideg) + ddot(dimen0, vec(1:dimen0), 1, psi(1:dimen0,ktau), 1)
           enddo
         enddo
      enddo

      if (job%verbose>=3) then
        write(out, '(1x,i6,1x,i2,1x,a,5x,i3,1x,i3,1x,a,5x,<nclasses>(1x,a),1x,<nmodes>(1x,i3),5x,f16.8,5x,<rank>(es16.8))') &
        ilevel, ideg, sym%label(isym), Jrot, eigen(ilevel)%krot, eigen(ilevel)%cgamma(0), eigen(ilevel)%cgamma(1:nclasses), eigen(ilevel)%quanta(1:nmodes), &
        energy-intensity%ZPE, me(1:rank,ideg)
      endif

    enddo ! ideg

    tav(:,jind,isym) = tav(:,jind,isym) + me(:,1) * intensity%gns(isym)*real(2*Jrot+1,rk)/intensity%part_func * exp(-(energy-intensity%ZPE)*beta)
    partfunc = partfunc + exp(-(energy-intensity%ZPE)*beta) * intensity%gns(isym)*real(2*Jrot+1,rk)

  enddo ! ilevel

  write(out, '(/3x,a,1x,a,6x,a)') 'J', 'gamma', 'thermal average'

  do jind=1, nJ
    do isym=1, sym%Nrepresen
      write(out, '(1x,i3,3x,i3,3x,<rank>(1x,es16.8))') Jval(jind), isym, tav(1:rank,jind,isym)
    enddo
  enddo

  write(out, '(/a,1x,i2,1x,a,1x,i2,1x,a,1x,f20.12)') 'partition function for J=[', intensity%J(1), ':', intensity%J(2), ']', partfunc

  deallocate(me, psi, tav, ijterm, vec)

  write(out, '(/a)') 'tav_eig: done'

end subroutine tav_eig
